...
EOF
    T=$(gettop)
    for i in `cat $T/build/envsetup.sh $T/vendor/bitter-polonium/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      echo "$i"
    done | column
}
...
function build_build_var_cache()
{
    ...
    cached_vars=`cat $T/build/envsetup.sh $T/vendor/bitter-polonium/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
    cached_abs_vars=`cat $T/build/envsetup.sh $T/vendor/bitter-polonium/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^bp_") ; then
        CM_BUILD=$(echo -n $1 | sed -e 's/^bp_//g')
...
function lunch()
{
    ...
    then
        ...
        T=$(gettop)
        cd $T > /dev/null
        vendor/bitter-polonium/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/bitter-polonium/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/bitter-polonium/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
...
. $ANDROID_BUILD_TOP/vendor/bitter-polonium/build/envsetup.sh
